...
time = np.genfromtxt(filename, delimiter=",", usecols=0, dtype="str")
timeDT = []
for x in time:
    timeDT.append(dt.datetime.strptime(x, "%d.%m.%Y %H:%M:%S"))
conc = np.genfromtxt(filename, delimiter=",", usecols=1)
satflow = np.genfromtxt(filename, delimiter=",", usecols=2)
...
calibSat = np.genfromtxt(calibFilename, delimiter=",", usecols=0)
calibDia = np.genfromtxt(calibFilename, delimiter=",", usecols=1)
calibDet = np.genfromtxt(calibFilename, delimiter=",", usecols=2)
...
while i < len(diaCut)-1:
    dlogdp.append(np.log10(diaCut[i]*1e-9)-np.log10(diaCut[i+1]*1e-9))
    i += 1
...
i=0
with open('{}-PSMconcDN.txt'.format(dataDay), 'w') as fp:
    fp.write("{}\n".format(",".join([str(y) for y in diaCut])))
    while i<len(timenew_avg):
        fp.write("{},{}\n".format(timenew_avg[i], ",".join([str(y) for y in conc4[i]])))
        i += 1

i=0
with open('{}-PSMconc.txt'.format(dataDay), 'w') as fp:
    while i<len(timenew_avg):
